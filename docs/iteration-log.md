...
### #edit-tool-trap — Edit 工具的坑
- 06-29 CSS 替换静默失败（缩进/换行不匹配）
- 06-20 CSS 解析全炸（吞 `}`）
- 06-23 linter 频繁改文件，LF→CRLF 导致 Edit 连续报 "file modified"

### #process-zombie — 进程堆积 / 孤儿
- 06-24 cloudflared 140+ 实例爆炸
- 06-15~16 MCP 僵尸进程（92 datetime + 88 browser + …）
- 06-11 10 个旧 Claude CLI 残留

### #import-corruption — 代码被自动修改
- 06-27~28 linter 自动格式化破坏 import（`resolveModelKey` → `WebaodelKey` typo）
- 06-29~30 `selectSidebarModel` 16 行被误删
- 06-15~16 `const`/`var` 重复声明 → SyntaxError → 前端完全冻结

### #turn-gate — Turn 调度锁死
- 06-12 Turn Gate 永久锁死（无超时机制）
- 06-16 `abandonStuckTurn`：turn 长时间卡住无人释放

### #css-silent-fail — CSS 静默失败
- 06-20 `*` reset 冲突、inline style 覆盖 class
- 06-23 inline `display:none` 覆盖 `.show` class
- 06-16 贴纸面板手机端常显 + 关不掉

### #windows-quirk — Windows 特有坑
- 06-24 `Get-CimInstance` 命令行截断 / 格式不同
- 06-15~16 `shell: true` → 四层进程链脆弱
- 06-16 Windows Service `LocalSystem` 读不到用户配置

### #contract-mismatch — 两端没对齐的契约
- 06-24 HEAD `/healthz` → 404（只匹配 GET）
- 06-24 `-TimeoutSecs` vs `-TimeoutSec`、`-AsHashtable` PS5.1 不兼容
- 06-24 `-Wait` 阻塞 → watch 循环从未运行（死代码）

### #vps-deploy — VPS 部署相关
- 06-30 VPS ENOENT 风暴（Windows 路径在 Linux 上不存在）
- 06-30 system message 静默跳过（`allowSpawn=false`）
- 06-25 SSH 初始连不上、端口 22 被运营商封
...
- **Phase 1（新增）：今日碎片质量审查**
  - 传入当天**全部**碎片（不只是高温的），附带标签状态和 ⚠️短 标记
