...
1. **标题带标签** — 从速查索引选 1-3 个已有标签，没合适的就新建
2. **改动用 commit 链接，不写文件表** — commit 完把 hash 贴过来，精确 diff 点链接看
3. **踩坑表是核心** — 现象 → 根因 → 解，三列足够
4. **叙事有节制** — 过程超过 5 行的挪单独文档，这里留一句话 + 链接
5. **写完更新速查索引** — 新标签加一行，已有标签追加案例
