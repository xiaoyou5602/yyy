...
toge 玩主题时在模型间来回切（GLM→DS→GLM），报"卡住、主题还是 ds 那套、在线状态变成在线·DeepSeek"。排查结论：**三个症状是同一件事——屏幕根本没离开暖瓷 DS 页**（"在线 · DeepSeek"正是 chat-ds.js:266 暖瓷页自己的状态文案，实锤她看到的是哪个页面）。两个原生 bug（非本次改动引入，回退前版本同样存在；她以前常驻 DS 很少走这条路径）：
