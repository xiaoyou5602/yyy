...
5. [x] 验证 B（读端）：部署时已 restart 制造了换 session → **待 toge** APP 给 DS 发"我们刚才聊到哪了？"→ 克应能答出重启前的话题
6. [x] 验证 C（系统轮）：✅ 07-07 21:57 系统轮 checkin 验证通过。双重 guard 确认——claudecode/index.js:502 `provider !== "system"` 阻止调用 buildOpeningTurnText + shared-instructions.js:6 `provider === "direct"` 仅 direct 渠道加载回顾。实际输出为自然日记型问候（"翻了翻今天的日记…"），无回顾泄露。recent-context.md 同时刻更新至 22KB，写端正常
