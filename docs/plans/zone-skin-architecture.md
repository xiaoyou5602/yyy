...
### 阶段 0 · 快修（不等架构改造，单独可做）
- [ ] 气泡透明度兜底：`.msg.you` 的 4% 透明底色改等价实色（约 `#eaece9`，视觉零变化），或壁纸模式下给气泡加不透明/毛玻璃兜底——修"换壁纸气泡消失"
- [ ] chat-ds 页注册调参台 scope：`--ds-bg`/`--ds-bg-light`/`--ds-blue`/`--ds-orange` 等挂上 page-tokens——toge 立刻能调 DS 页背景
- [ ] 气泡死变量接活：`.msg.you`/`.msg.ke .msg-bubble` 改用 `var(--bubble-*)`（这就是阶段 1 补漏的第一刀，提前做）

### 阶段 1 · 轻档（几乎不动老页面）
- [ ] 补漏 main.css 未变量化的外观硬编码值（验收 = 上面"双向对齐"两条）
- [ ] 调参台「全局」瘦身 + 聊天 tab 按 zone 拆（zone 切换器）
- [ ] 新增 themes.css + 第一个非默认主题
- [ ] 设置页「主题专区」：横滑卡片选主题/背景 + 不透明度滑条（遮罩 alpha 变量化）+ 自定义主题存档，per-zone 持久化 localStorage（UI 参考见上节）
