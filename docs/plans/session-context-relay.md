...
> 状态：**待开工**（2026-07-04 立案，toge 已确认方向）
> 预估工作量：一个晚上（核心 2 个文件改动 + 验证）
...
1. [ ] 写端：`recent-context-writer.js`（生成函数 + 防抖）+ direct adapter 挂钩子（~1h）
2. [ ] 读端：shared-instructions.js 加 `loadRecentContext` + buildOpeningTurnText 注入段（~30min）
3. [ ] 本地语法检查 `node --check`，commit + push vps/github + VPS pull + **restart cyberboss**
4. [ ] 验证 A（写端）：VPS 上给 DS 发一条消息，5 秒后 `cat /root/.cyberboss/recent-context.md`，确认内容与过滤正确（无 thinking/JSON/错误条目）
5. [ ] 验证 B（读端）：`systemctl restart cyberboss`（故意制造换 session）→ APP 给 DS 发"我们刚才聊到哪了？"→ 克应能答出重启前的话题
6. [ ] 验证 C（系统轮）：等一次 checkin 触发，确认日志里系统轮 opening turn 没带回顾（`provider=system`）
7. [ ] WITHTOGE.md 待办打勾 + 迭代日志补一笔
