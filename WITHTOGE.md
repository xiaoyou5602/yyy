...
```
手机 APK ──→ Cloudflare Tunnel ──→ cyberboss 服务 ──→ claudecode runtime ──→ DeepSeek API
电脑网页 ──→ 127.0.0.1:9726 ──┘      │
                                      ├── 日记 / 时间轴 / 记忆碎片
                                      ├── 世界书 / 礼物 / 摄像头 / MCP
                                      └── 闹钟 / 贴纸 / 定位 / 图片识别 / 奶茶记录
...
| **视觉调参台**        | CSS 变量实时调节                                            | `src/adapters/channel/direct/client/js/tweak.js`                                          |
| **PWA**               | Service Worker 离线缓存                                     | `src/adapters/channel/direct/client/sw.js`, `manifest.json`                               |
| **Session 管理**      | claudecode runtime，启动清旧 session                        | `src/adapters/runtime/claudecode/`                                                        |
| **主动问候**          | checkin poller，随机间隔发消息                              | `src/app/system-checkin-poller.js`                                                        |
...
| **06/16** | 贴纸系统完整接入 APP/Web：6 个 API + WS 贴纸消息 + 前端贴纸面板/更多面板/标签编辑/上传删除。修 5 个 bug（面板常显、GIF 裁切、上传缓存、前端冻结、信件跳转）。cloudflared 注册为 Windows Service 独立生命周期。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
