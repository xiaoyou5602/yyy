...
端口：**9726**（`0.0.0.0` 监听）
**VPS**：东京 LocVPS `103.85.25.226`，Ubuntu 22.04，2核4G，¥36/月，systemd 守护 3 服务（cloudflared / cyberboss / notion-mcp）
**Notion MCP**：`https://notion.withtoge.us`，7 工具，端口 3000，cloudflared ingress 路由
Android APK：`C:\Users\youzi\Desktop\克-apk\克-v12.apk`，versionCode 12，纯全屏 WebView，硬编码 `https://克.withtoge.us`

### 技术避坑

- **生产环境已搬 VPS，systemd 守护**。本地 `npm run safe` 和 guardian PowerShell 脚本仅保留作为历史参考，不要在生产中使用
- **不再需要手动管理 cloudflared**。VPS 上由 systemd 的 `cloudflared.service` 自动管理
...
| 审批弹窗            | ✅ 已修复 — 华为 WebView CSS GPU 合成层死锁（backdrop-filter + animation），去掉后正常。同时加 pendingApprovals 轮询兜底、model 感知、双发文本消息 |
