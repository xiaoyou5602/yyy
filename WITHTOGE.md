...
- [ ] 聊天页独立 UI / 皮肤架构（07-05 立项）— **阶段 1~4 全部完成（07-10）**。阶段 1 toge 验收通过；阶段 2~4（04e9b3f/ef2e1da/2a78cd5）：渲染抽成皮肤查表转发 + zone 模板化（加模型一行做实）+ skin-dev 沙盒 + 暖瓷回收成 ceramic 皮肤、**DS 默认接回主引擎**（应急阀：设置页「DS 引擎切换」回旧暖瓷页）。**阶段 2~4 + chatnest 皮肤（96ae7c8）待 toge 实机验收**——注意 ① 多段回复改按 chunk 拆气泡 ②DS 出问题设置页一键切回旧暖瓷页 ③Rism/opus 已穿 chatnest 仿官端（暗色跟系统；header 想一起暗就给 opus 挂午夜主题）。首次实现（07-07 34f890c）因三架构问题于 07-10 凌晨回退；Fable 接手重做：调参台按 zone 拆 scope（token 直落 #chat-zone-\*，废除 #chat-page 共享落点）+ tweak 按主题分桶（切主题清 inline 重应用，主题永不被碾压）+ DS 暖瓷页 CSS 桥接跟主题 + themes.css 4 主题 + 主题专区页 + 「存为主题卡片」闭环。本地端到端验证全过（隔离/header 跟随/分桶/桥接/存卡）。阶段 2~4 待排。细节 → docs/plans/zone-skin-architecture.md
- [x] 暖瓷页切非 DS 模型卡死（07-10 toge 报：glm→ds→glm 卡住、主题还是 ds 那套、状态"在线·DeepSeek"）— **已修（07-10 commit 1312832，已部署）**。三症状=屏幕没离开暖瓷页这一件事。两个原生 bug：①selectSidebarModel 非 DS 分支不 showPage("chat")，zone 在被盖住的页面里切了但不揭幕；②DS 分支不 activateZone("ds")，refs 停在旧 zone，下次切走时旧 zone 历史会覆写 DS 本地历史 key（暖瓷页硬编码读的那个）。本地复现三步序列验证全过，toge 实机验证通过（07-10）。**注意"在线 · DeepSeek"字样=正在看暖瓷页**，以后排查页面错乱先认这个标记
...
- [ ] 迁移验收 — 按 orangechat/README.md 第四节清单过一遍（日记/奶茶/自动同步/VPS 状态/摘要/heat 衰减）
- [ ] Phase 3 记忆迁移（不急）— IDE 端 supabase-memory.js CLI + legacy diary/memory md → Supabase 导入脚本 + 世界书 10 条目内容从 memory 文件提炼
