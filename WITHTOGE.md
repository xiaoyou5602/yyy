...
- [ ] 调参台失准三病根（07-05 toge 报）— 已查实：气泡 token 是死变量（真实底色硬编码在 `.msg` 规则）、DS 页（#chat-ds-page）不在 token 注册表、气泡 4% 透明度壁纸下隐形。方案并入 [docs/plans/zone-skin-architecture.md](docs/plans/zone-skin-architecture.md) 调参台节 + 新增阶段 0 快修（可先行单独做）
