...
| **06/24**    | **cloudflared 1033 根治**：guardian 全面重写（PID 三重校验+分层健康检查+退避持久化+熔断+断连快照），铲除 `src/index.js` 内部 spawn 根源，PS5.1 API 兼容修复，kill-zombies 加固（age 宽限+杀后验证），`/healthz` 端点 + 启动自检根治三次同型死代码 bug。通知系统三连修补（延迟+页内弹出+掉线在线）APK v13 |
