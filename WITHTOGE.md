...
- [x] 审批弹窗 — 已修复。华为 WebView CSS GPU 合成层死锁（backdrop-filter + animation），去掉后正常。同时加 pendingApprovals 轮询兜底、model 感知、双发文本消息
- [ ] 调参台完善 — 待验证。所有页面独立微调（13 个 tab），点击遮罩关闭，scrollbar 可见。重启后 toge 自己验证
- [ ] 三模型页面拆分 — 把当前三个模型页面拆成独立三页，各自不同主题色，可自己画配件/垫图装饰
- [x] 设置页去"我的昵称" — 已修。删掉设置里的昵称项，统一从世界书取
- [x] 世界书 ai 名字同步 — 已修。世界书保存/加载/切模型时自动同步标题，每个模型独立
...
- [x] COT 内行距缩短 — 已修。line-height 1.5→1.3，事件委托防点击失效，指纹精确恢复防消失
- [x] COT 突然消失 — 已修。根因是 loadCachedThinking 盲匹配+全删重建，改为 msgFingerprint 精确恢复 + 事件委托

### 后端 / 服务

- [ ] 闹钟接入聊天流程 — parser 和 APK 已就绪，需接到 Claude Code 对话里
- [ ] 僵尸进程自动清理 — kill-zombies.ps1 已重写，待 Codex 审核后测试
- [ ] 废弃微信 MCP 工具下线 — tool-mcp-server 仍暴露已废弃的微信通道工具，IDE 端会话会误调。从 MCP server 移除或标记 deprecated
- [ ] 前端组件化 — 记忆/涂鸦/桌宠组件化
- [ ] 隧道大数据响应断开 — WebSearch 多轮结果堆积，响应体过大时 cloudflared 隧道断开，手机端超时后不自动重连

### 克的行为优化

- [ ] 记忆碎片：对话版 skip [🟢低] — 对齐日记版质量门控，过滤纯时间线、Markdown 噪音等，改动约 1-2 行
- [ ] 记忆碎片：无主语短句 [🟢低] — 识别隐含主语的短句，无主语+身份关键词→自动归类 identity，改动约 5-10 行
- [ ] 记忆检索精度优化 [🟡中] — 每句话不盲目捞碎片，只捞跟当前话题相关的，调检索精度不调量
