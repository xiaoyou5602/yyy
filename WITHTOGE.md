...
- [ ] 梦境系统 07-02 起每晚崩（07-05 诊断）— `consolidation-scheduler.js` runDailyDream 里 `readByDate()` 是 async 但没 await（07-01 大修 010e3a2 引入），凌晨 3 点必报 `todayFrags.slice is not a function`，打标签/质量整理/热度衰减/rollup/写信全部停摆。修法：加 await
- [ ] resolveModelKey("glm") 落回 ds（07-05 诊断）— `config.js` 直通名单只有 ds/opus/haiku，`getModelMemoryDir(stateDir,"glm")` 返回 ds 目录 → glm 的记忆碎片/worldbook 与 ds 混用（VPS 上 memory/glm 不存在）。修法：名单改用 ALL_MODEL_KEYS。同病：scheduler `modelKeyToModelName` 缺 glm/openclaw，梦境消息会以 model="" 发给 ds
- [ ] 清理 haiku/opus 的日记幻影碎片（07-05 诊断）— haiku 9 个碎片文件全是 diary 来源（07-01 加 hasChatActivity 闸之前落下的存量），haiku 已从路由表退役，可整目录删；opus 目录混少量真实测试 chat，只删 diary 来源需逐条筛
