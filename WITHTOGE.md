...
- [x] 审批弹窗无人响应导致 turn 挂死（07-04 发现）— **已修（07-04 commit f7a926a，已部署）**。背景：19:27 一轮连弹 3 个审批，第 3 个没人点，turn 挂 1061s 才被 turn-gate 兜底清理。修法：审批弹窗发出后挂 2 分钟定时器，超时且 `pendingApproval` 仍是同一 requestId → 自动 decline + 聊天页留一条"⏰ 超时自动拒绝（非 toge 主动）"说明（进历史，克跨 session 回顾可见）。被响应/turn 结束/换新审批都会清 pendingApproval，回调天然幂等。时长 2 分钟是 toge 07-04 拍板的
...
- [x] session 重启自动接续上下文（07-03 toge 报）— **已修（07-04 commits 36d750e/8ffad02/06d2b97，已部署 VPS）**。三件套：①写端 `recent-context-writer.js` 消息落库后防抖 5s 自动重写 `~/.cyberboss/recent-context.md`（24h/8000 字/60 条，过滤 thinking/JSON/错误/审批弹窗，checkin 主动消息标「（主动）」）；②读端 opening turn 注入回顾段（handoff 手动通道保留）；③计划外关键发现——`session_replaced` 场景 openingTurn=false 根本不走 opening 注入（新 session 连 persona 都没有），claudecode/index.js opening 条件加 `|| !hadExistingSession`，重启后首条消息必带完整注入。踩坑：存储 model 字段是 modelName（"deepseek-v4-pro"）不是路由键 "ds"，空 model = checkin 主动消息。**07-04 第一次验收失败已修（commit 6570745）**：checkin 系统轮 spawn 的裸 session 抢在 toge 前面（5 分钟 idle 清理未到），她的首条消息 attach 上去时两个 opening 条件全绕过——克只有 checkin 上下文（暴露点：它引用了只有系统轮才有的健康数据）。修法：session 记 `userTurnSeen`，用户首条消息进入无用户轮的 session 强制 opening 注入。**待 toge 再验证**：问 DS「凌晨 1:26 发了什么」应能答出。claudecode `--resume` 为何总失败另行调研（改动大、边界多，暂缓；就算修好，重启断片依然需要回顾兜底）。方案与排查全文 → [docs/plans/session-context-relay.md](docs/plans/session-context-relay.md)
