...
- [x] 调参台失准三病根（07-05 toge 报）— **阶段 0 快修已做（07-05，未部署）**：`.msg.you`/`.msg.ke` 气泡改用 `var(--bubble-*)` 激活死变量 + `--bubble-you` 透明底色改等价实色 `#eaece9`（壁纸下不再隐形）+ page-tokens.js 新增 `chat-ds` scope 注册 DS 页 6 个 token。详见 [docs/plans/zone-skin-architecture.md](docs/plans/zone-skin-architecture.md) 阶段 0。**待部署 VPS + toge 实机验收**
