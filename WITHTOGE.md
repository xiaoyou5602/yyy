...
- [ ] 自搭 DS Agent Loop（07-09 立项）— **方案已定稿 → [docs/plans/ds-agent-loop.md](docs/plans/ds-agent-loop.md)**（07-10 第四轮修订：补事件协议/失败路径/close 中断语义/`/compact` 拦截）。**步骤 0 端点实测已完成（07-10）**：七场景夹具入库 `test/fixtures/ds-sse/`，最大发现=DS thinking 模式强制同轮回传 thinking block（原伪代码写法会 400），详见计划 §5.10。分工（toge 07-10 拍板）：核心两文件（stream-parser + agent-client）Fable 亲写，外围（messageStore 扩展/集成/部署）可交其他模型。⚠️ 本地 .env 的 DEEPSEEK_KEY 已失效，有效 key 只在 VPS，复跑抓取脚本走 VPS
