...
- [ ] 自搭 DS Agent Loop（07-09 立项）— **代码侧全部完成，只差部署验证（07-10）**。方案 → [docs/plans/ds-agent-loop.md](docs/plans/ds-agent-loop.md)。已落地：①步骤 0 端点实测（七场景夹具 `test/fixtures/ds-sse/`，最大发现=thinking 模式强制同轮回传 thinking block，只回 tool_use 会 400）②核心两文件 `ds-stream-parser.js`+`ds-agent-client.js`（Fable 亲写，21 项单测全绿，含审批时序 bug 修复：waiter 必须先注册再 emit）③集成分流（commit f182ce1，claudecode-approval 存量测试对照零回归）。**应急阀：`CYBERBOSS_DS_AGENT_LOOP=off` 一键回退 CLI 子进程**。待做：部署验证（重启 cyberboss，挑 toge 不在聊的时机；验证清单见计划 §9）+ 二期 messageStore 扩 tool_call/tool_result 落库（外围，不阻塞对话）。⚠️ 本地 .env 的 DEEPSEEK_KEY 已失效，复跑抓取脚本走 VPS
