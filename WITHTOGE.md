...
- [ ] 自搭 DS Agent Loop（07-09 立项）— **方案已定稿 → [docs/plans/ds-agent-loop.md](docs/plans/ds-agent-loop.md)**。目标：自建 HTTP 直调 DS API 的 agent loop 替换 Claude CLI 子进程，省掉 Anthropic 官方几万字 system prompt 的 token 浪费。核心风险：tool_use 的 SSE 状态机需新写（非复用）；现在用 VS Code 插件翻看 DS session 靠的是 Claude CLI 自动写盘到 `/root/.claude/projects/-root/`，自建后这条链路会断供，MVP 期先用简化落盘顶着，第二阶段再补
