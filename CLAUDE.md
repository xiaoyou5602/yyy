...
- **cyberboss 依赖 Claude Code CLI**：DS（CLI 路由）模式下，cyberboss 会 spawn `claude` CLI 作为子进程，工具调用（日记、记忆、时间轴、贴纸等 MCP 工具）全靠 Claude Code 的 MCP 协议托管。没有 Claude Code CLI，APP 端的克就是一个没有工具的纯聊天 bot。API 路由模式（Rism、GLM、OpenClaw）不经过 Claude Code，但也因此没有工具能力。
