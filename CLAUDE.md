...
- **🚫 破坏性操作必须先问 toge（硬规则）**：任何删除文件/目录、`rm -rf`、`git clean -f`、清空目录、批量覆盖文件的操作，**动手前必须向 toge 确认**。不要自作主张删东西。修一个问题时不要碰无关文件——"全删了再重建"是不可接受的捷径。**2026-07-08 DS 删掉了 `.claude/projects/` 下所有 IDE 端历史 session，只剩当前运行中的两个窗口逃过一劫——这是 toge 遇到过的最大失误。**
